...
export default ({ id }: BuildingProps) => {
  const building = GameData.building(id);

  const completed = useAppSelector(state => state.checklist.items[building.id] ?? false);
  ...
  const setBuildingOpen = (evt: React.ToggleEvent<HTMLDetailsElement>) => {
    ...
    dispatch(setUiElementHidden({ id: building.id, hidden: !evt.currentTarget.open }));
  }
  const onCompleted = (evt: React.ChangeEvent<HTMLInputElement>) => {
    dispatch(setCompleted({ id: building.id, completed: evt.currentTarget.checked }));
  }
  ...
  return (
    <details className="border rounded-xl bg-slate-200 max-w-xl" open={expanded} onToggle={setBuildingOpen}>
      <summary className="text-xl font-bold space-x-4 ml-4 h-8">
        ...
        <a className="align-middle" href={building.wiki} target="_blank">{building.label}</a>
      </summary>
